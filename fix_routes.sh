#!/bin/bash
# Run this from your project root: bash fix_routes.sh

echo "=== Fixing AdminLayout.vue ==="
cat > resources/js/components/Admin/AdminLayout.vue << 'ENDOFFILE'
<template>
...
ENDOFFILE
echo "✓ AdminLayout.vue fixed"

echo "=== Fixing RescheduleModal.vue ==="
sed -i "s|await axios.patch(route('admin.bookings.reschedule', props.bookingId), form.value)|await axios.patch(\`/admin/bookings/\${props.bookingId}/reschedule\`, form.value)|g" \
  resources/js/components/Admin/Modals/RescheduleModal.vue
echo "✓ RescheduleModal.vue fixed"

echo "=== Fixing QuickBooking.vue ==="
sed -i "s|<Link :href=\"route('user.login')\" class=\"ml-1 underline\">Login for faster booking</Link>|<a href=\"/login\" class=\"ml-1 underline\">Login for faster booking</a>|g" \
  resources/js/pages/Landing/QuickBooking.vue
sed -i "s|<Link :href=\"route('booking.track', bookingNumber)\"|<a :href=\"\`/track/\${bookingNumber}\`\"|g" \
  resources/js/pages/Landing/QuickBooking.vue
sed -i "s|<Link :href=\"route('home')\"|<a href=\"/\"|g" \
  resources/js/pages/Landing/QuickBooking.vue
# Fix closing Link tags in QuickBooking success section
sed -i 's|Track Your Booking\n            </Link>|Track Your Booking\n            </a>|g' \
  resources/js/pages/Landing/QuickBooking.vue
sed -i 's|Back to Home\n            </Link>|Back to Home\n            </a>|g' \
  resources/js/pages/Landing/QuickBooking.vue
echo "✓ QuickBooking.vue fixed"

echo "=== Fixing Dashboard.vue ==="
sed -i "s|:href=\"route('admin.bookings.create')\"|href=\"/admin/bookings/create\"|g" \
  resources/js/pages/Admin/Dashboard.vue
sed -i "s|:href=\"route('admin.bookings.index')\"|href=\"/admin/bookings\"|g" \
  resources/js/pages/Admin/Dashboard.vue
sed -i "s|:href=\"route('admin.bookings.show', booking.id)\"| :href=\"\`/admin/bookings/\${booking.id}\`\"|g" \
  resources/js/pages/Admin/Dashboard.vue
sed -i "s|:href=\"route('admin.consultations.index')\"|href=\"/admin/consultations\"|g" \
  resources/js/pages/Admin/Dashboard.vue
sed -i "s|:href=\"route('admin.technicians.index')\"|href=\"/admin/technicians\"|g" \
  resources/js/pages/Admin/Dashboard.vue
echo "✓ Dashboard.vue fixed"

echo "=== Adding markInProgress to BookingService ==="
sed -i "s|    public function cancelBooking(|    public function markInProgress(Booking \$booking, \$actor): Booking\n    {\n        return \$this->bookingRepo->updateStatus(\$booking, 'in_progress', 'Service started', \$actor);\n    }\n\n    public function cancelBooking(|g" \
  app/Services/BookingService.php
echo "✓ BookingService.php fixed"

echo ""
echo "=== Verifying no route() calls remain ==="
grep -rn "route(" resources/js/ --include="*.vue" | grep -v "//.*route(" | grep -v "isActive\|page.url"
